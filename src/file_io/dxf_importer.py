...
from geometry.topology import TopologySolver
...
class DXFImporter:
    ...
    def __init__(
        ...
        self.topology_solver = TopologySolver(tolerance=tolerance)
    ...
    def import_file(self, filepath: str) -> List[Polygon]:
        ...
        try:
            ...
            # Group connected segments into shapes (using topology solver)
            shapes = self.topology_solver.group_segments(entity_segments)
            ...
            for i, shape_points in enumerate(shapes):
                ...
                try:
                    ...
                    if polygon.is_valid() and polygon.area > 0.1:
                        # Normalize polygon to origin (0,0) for consistent placement
                        normalized_polygon = polygon.normalize_to_origin()
                        polygons.append(normalized_polygon)
                        self.stats.shapes_created += 1
                except Exception as e:
                    self.stats.errors.append(f"Shape {i}: {str(e)}")
            
            # Detect and associate holes
            if len(polygons) > 1:
                polygons = self.topology_solver.detect_holes(polygons)
